...
void ExampleRenderer::initialiseAssests() {

	auto loadFunc = [](const string& name, PS4Mesh** into) {
		*into = new  PS4Mesh(name);
		(*into)->SetPrimitiveType(GeometryPrimitive::Triangles);
		(*into)->UploadToGPU();
	};
	loadFunc("cube.msh", &cubeMesh);
	loadFunc("sphere.msh", &sphereMesh);
	basicTex = PS4Texture::LoadTextureFromFile("/app0/Assets/Textures/doge.gnf");
	basicShader = defaultShader;

}

void ExampleRenderer::Initworld() {
...
void ExampleRenderer::Update(float dt)	{
	...
	defaultObject[0]->SetLocalTransform(Matrix4::Translation(Vector3(-0.4, 0, 0)) * Matrix4::Rotation(*computeResult, Vector3(0,0,1)));
